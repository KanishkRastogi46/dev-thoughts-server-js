  ...
  async register(registerDto: RegisterDto): Promise<number> {
    try {
      ...
      const profile: Profile = {
        ...
        code: registerDto.countryCode,
      };
      const profileId = await this.authRepository.createProfile(profile);
      if (profileId.length === 0) {
        this.logger.error('Failed to create user profile');
        throw new BadRequestException('User profile creation failed');
      }

      const otp = this.authRepository.createOtp(
        generateOTP(),
        userId[0].id,
        'VERIFY ACCOUNT',
      );
    ...
    try {
      ...
      const refreshToken = generateToken(
        ...
        this.configService.get<string>('JWT_REFRESH_SECRET_KEY'),
        {
          algorithm: this.configService.get('JWT_ALGORITHM'),
          expiresIn: parseInt(
            this.configService.get('JWT_REFRESH_EXPIRY_TIME'),
          ),
          issuer: this.configService.get('JWT_ISSUER'),
          audience: this.configService.get('JWT_AUDIENCE'),
        },
      ...
      generateCookie(res, 'refreshToken', refreshToken, {
        ...
        secure: this.configService.get<string>('NODE_ENV') === 'production',
        maxAge:
          this.configService.get<number>('JWT_REFRESH_EXPIRY_TIME') * 1000,
        sameSite: true,
      });
    ...
    try {
      ...
      if (userId) {
        ...
        if (accessToken) {
          ...
          generateCookie(req.res, 'accessToken', newAccessToken, {
            httpOnly: true,
  ...
  async resendOtp(userId: number) {
    try {
      ...
      if (!canResendOtp) {
        ...
        throw new BadRequestException(
          'Cannot resend otp now. Please try again later.',
        );
      }
      ...
      throw new HttpException(
        error.message || 'Otp can be send only thrice in 1 hour',
        error.status || 500,
      );
    }
  }

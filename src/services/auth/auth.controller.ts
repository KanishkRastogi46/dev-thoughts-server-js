...
export class AuthController {
  ...
  async validateOtp(
    ...
    const validateOtpResponse = await this.authService.validateOtp(
      userId,
      otpDto.otp,
    );
  ...
  @Post('refresh-token')
  @HttpCode(HttpStatus.OK)
  async refreshAuthToken(@Req() req: Request, @Res() res: Response) {
  ...
  async forgotPassword(
    ...
    const forgotPasswordResponse = await this.authService.forgotPassword(
      body.email,
    );
  ...
  async newPassword(
    ...
    const userId = parseInt(req.headers['x-user-id'] as string);
    const newPasswordResponse = await this.authService.newPassword(
      userId,
      body.password,
    );

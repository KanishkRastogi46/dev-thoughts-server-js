...
export const commentsTable = pgTable('comments', {
  ...
  post: integer('post').references(() => postsTable.id, {
    onDelete: 'cascade',
  }),
  profile: integer('profile').references(() => profileTable.id, {
    onDelete: 'cascade',
  }),
  text: text('text').notNull(),
  isReply: boolean('is_reply').default(false).notNull(),
  parentComment: integer('parent_comment').references(() => commentsTable.id, {
    onDelete: 'cascade',
  }),
